<template>
  <v-row class="mt-2">
    <v-col class="d-flex justify-center aling-center">
      <v-card :loading="isLoading" min-width="400">
        <v-list three-line>
          <template v-for="(item, index) in users" :kry="item.id">
            ...
            <v-list-item router :to="{ name: 'user', params: { id: item.id } }">
              <v-list-item-avatar>
              ...
              <v-list-item-content>
                <v-list-item-title
                  v-html="item.last_name + ' ' + item.first_name"
                ></v-list-item-title>
...
export default {
  ...
  components: { UserInfo },
  data() {
    return { users: {}, header: "Users", isLoading: true };
  },
